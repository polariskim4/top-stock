...
def get_kr_top_performers(year_start, year_end, market_type): # market_type 인자 추가
    ...
    # 'Sector' 컬럼 존재 여부 확인 후 필터링 (KRX API 응답 변경 대비)
    ...
    tickers_map = {row[code_col] + suffix: row['Name'] for _, row in df_krx.iterrows()}
            ...
            try:
                ...
                if not data.empty:
                    ...
                    for ticker in temp_df.columns:
                        ...
                        # 데이터가 충분하고 가격 변동이 있는 종목만 (거래정지 제외)
                        if len(ticker_series) > 10 and ticker_series.std() > 0:
                            s_price = float(ticker_series.iloc[0])
                            e_price = float(ticker_series.iloc[-1])
                            if s_price > 0:
                                ret = ((e_price / s_price) - 1) * 100
                                results.append({'종목코드': ticker.split('.')[0], '종목명': tickers_map[ticker], '수익률(%)': round(ret, 1)})
            except:
                continue
...
def get_us_top_performers(year_start, year_end, market_type):
    ...
    with st.spinner(f"미국 {market_type} 리스트 구성 중..."):
        ...
        if market_type == 'S&P 500':
            df_listing = fdr.StockListing('S&P500')
            us_names_map = dict(zip(df_listing['Symbol'], df_listing['Name']))
            ...
            us_names_map = dict(zip(df_listing['Ticker'], df_listing['Company']))
            ...
            us_names_map = dict(zip(df_listing['Symbol'], df_listing['Company']))

    # yfinance 호환성을 위해 티커 기호 정제 및 이름 맵 정제
    all_us_tickers = [str(ticker).replace('.', '-') for ticker in us_names_map.keys()]
    us_names_map = {str(k).replace('.', '-'): v for k, v in us_names_map.items()}
                ...
                if not data.empty:
                    ...
                    for ticker in temp_df.columns:
                        ...
                        if len(ticker_series) >= 2:
                            ...
                            if s_price > 0:
                                ret = ((e_price / s_price) - 1) * 100
                                results.append({'티커': ticker, '종목명': us_names_map.get(ticker, ticker), '수익률(%)': round(float(ret), 1)})
            except:
                continue
