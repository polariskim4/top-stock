...
def get_us_top_performers(year_start, year_end, market_type):
    ...
    with st.spinner(f"미국 {market_type} 리스트 및 상세 섹터 정보 구성 중..."):
        ...
        if market_type == 'S&P 500':
            # 위키피디아에서 S&P 500 상세 정보 추출 (Sub-Industry 활용)
            url = 'https://en.wikipedia.org/wiki/List_of_S%26P_500_companies'
            resp = requests.get(url, headers={'User-agent': 'Mozilla/5.0'})
            df_listing = pd.read_html(io.StringIO(resp.text), match='Symbol')[0]
            for _, row in df_listing.iterrows():
                us_info_map[row['Symbol']] = {'name': row['Security'], 'sector': row.get('GICS Sub-Industry', row.get('GICS Sector'))}
            ...
            for _, row in df_listing.iterrows():
                # 상세 섹터를 위해 GICS Sub-Industry 우선 순위
                us_info_map[row['Ticker']] = {'name': row['Company'], 'sector': row.get('GICS Sub-Industry', row.get('GICS Sector', 'Technology'))}
            ...
            df_listing = pd.read_html(io.StringIO(resp.text), match='Symbol')[0]
            for _, row in df_listing.iterrows():
                us_info_map[row['Symbol']] = {'name': row['Company'], 'sector': row.get('Industry', 'Industrial')}
