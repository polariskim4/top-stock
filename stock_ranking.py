...
def get_kr_top_performers(year_start, year_end, market_type): # market_type 인자 추가
    ...
    with st.status(f"{market_type} 데이터 수집 중...", expanded=True) as status:
        for i in range(0, len(all_tickers), batch_size):
            ...
            try:
                # yfinance 배치를 통해 속도 극대화
                data = yf.download(batch, start=year_start, end=year_end, progress=False)
                if not data.empty:
                    # 단일 종목과 다중 종목 결과 구조 통일
                    if len(batch) > 1:
                        temp_df = data['Close']
                    else:
                        temp_df = data[['Close']].rename(columns={'Close': batch[0]})
...
def get_us_top_performers(year_start, year_end, market_type):
    ...
    with st.status("미국 종목 분석 중...", expanded=True) as status:
        for i in range(0, len(all_us_tickers), batch_size):
            ...
            try:
                # yfinance 다운로드 (구조적 안정성 위해 Close 선택 로직 개선)
                data = yf.download(batch, start=year_start, end=year_end, progress=False)
                if not data.empty:
                    # 단일/다중 종목 결과 구조 통일
                    if len(batch) > 1:
