...
def get_us_top_performers(year_start, year_end, market_type):
    # 선택된 지수별 구성 종목 리스트 가져오기 (이미 개별주 위주로 구성됨)
    with st.spinner(f"미국 {market_type} 종목 리스트 구성 중..."):
        if market_type == 'S&P 500':
            all_us_tickers = fdr.StockListing('S&P500')['Symbol'].tolist()
        elif market_type == 'Nasdaq 100':
            all_us_tickers = fdr.StockListing('NASDAQ100')['Symbol'].tolist()
        elif market_type == 'Dow Jones':
            all_us_tickers = fdr.StockListing('DOW30')['Symbol'].tolist()
