...
def get_kr_top_performers(year_start, year_end, market_type): # market_type 인자 추가
    ...
    if 'Sector' in df_all.columns:
        ...
        # 종목 코드를 6자리 문자열로 규격화하여 매핑 정확도 향상
        sector_map = {str(row[all_code_col]).zfill(6): str(row['Sector']) for _, row in df_all.iterrows()}
    ...
    # 종목 정보 맵 (이름, 섹터)
    # 마켓 리스트의 코드도 6자리로 정규화하여 섹터 맵과 대조
    tickers_info = {str(row[code_col]).zfill(6) + suffix: {'name': row['Name'], 'sector': sector_map.get(str(row[code_col]).zfill(6), '기타')} for _, row in df_krx.iterrows()}
...
def get_us_top_performers(year_start, year_end, market_type):
    ...
    with st.spinner(f"미국 {market_type} 리스트 및 상세 섹터 정보 구성 중..."):
        # 1. S&P 500 정보를 먼저 가져와 상세 섹터(GICS Sub-Industry) 마스터 맵 생성
        # Nasdaq 100과 Dow 30 종목 대부분이 S&P 500에 포함되어 있어 가장 상세한 정보를 제공합니다.
        ...
        def extract_sector(row):
            # 가능한 섹터 관련 컬럼 리스트를 순차적으로 확인
            for col in ['GICS Sub-Industry', 'GICS Sector', 'Industry', 'Sector']:
        ...
        master_map = {str(row['Symbol']): {'name': row['Security'], 'sector': extract_sector(row)} for _, row in df_sp500.iterrows()}
