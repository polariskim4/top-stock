            ...
            try:
                ...
                if not data.empty:
                    ...
                    for ticker in temp_df.columns:
                        ...
                        # 윌비스, 디에이테크놀로지 등 최근 거래가 멈춘 종목은 tail(5).std()가 0이 됩니다.
                        if len(ticker_series) > 10 and ticker_series.std() > 0 and ticker_series.tail(5).std() > 0:
                            ...
                            if s_price > 0:
                                ...
                                # 정수로 끝나도 소수점 표시를 위해 데이터는 소수점으로 유지
                                results.append({'종목코드': ticker.split('.')[0], '종목명': tickers_map[ticker], '수익률(%)': float(ret)})
            except:
                continue
                ...
                if not data.empty:
                    ...
                    for ticker in temp_df.columns:
                        ...
                        if len(ticker_series) > 5 and ticker_series.tail(5).std() > 0:
                            ...
                            if s_price > 0:
                                ret = ((e_price / s_price) - 1) * 100
                                results.append({'티커': ticker, '종목명': us_names_map.get(ticker, ticker), '수익률(%)': float(ret)})
            except:
                continue
