...
def get_kr_top_performers(year_start, year_end, market_type): # market_type 인자 추가
    ...
    sector_map = {}
    # Sector 컬럼이 존재할 경우 매핑 딕셔너리 생성
    sector_col = 'Sector' if 'Sector' in df_all.columns else None
    code_col_all = 'Code' if 'Code' in df_all.columns else ('Symbol' if 'Symbol' in df_all.columns else None)

    if sector_col and code_col_all:
        # 종목 코드를 6자리 문자열로 규격화하고 섹터 정보의 공백 제거
        for _, row in df_all.iterrows():
            clean_code = str(row[code_col_all]).strip().zfill(6)
            sector_map[clean_code] = str(row[sector_col]).strip() if pd.notna(row[sector_col]) else '기타'
    ...
    for _, row in df_krx.iterrows():
        ...
        tickers_info[clean_code + suffix] = {
            'name': row['Name'], 
            'sector': sector_map.get(clean_code, '기타')
        }
...
def get_us_top_performers(year_start, year_end, market_type):
    ...
    with st.spinner(f"미국 {market_type} 정보 및 상세 섹터 구성 중..."):
        ...
        def extract_sector(row):
            # 컬럼 이름 대소문자 및 공백 무시하고 검색
            row_keys = {str(k).strip().lower(): k for k in row.index}
            # Sub-Industry를 우선하여 상세 분류(예: Semiconductors) 추출
            for target in ['gics sub-industry', 'gics sector', 'industry', 'sector']:
                if target in row_keys:
                    val = str(row[row_keys[target]]).strip()
